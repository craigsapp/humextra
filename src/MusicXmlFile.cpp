				...
				if (newlineQ) {
					if (humdrumDynamics && partdynamics[staffno]) {
						if (i < partdata[staffno].getSize()-1) {
							if (partdata[staffno][i].ticktime != partdata[staffno][i+1].ticktime) {
					...
					if (element->GetName() == "repeat") {
						for (i=0; i<element->GetAttributes().GetLength(); i++) {
							if (element->GetAttributes().GetName(i) == "direction") {
								if (element->GetAttributes().GetValue(i) == "backward") {
									repeatback = 1;
								}
							}
						}
						...
						while (obj2 != NULL) {
							if (obj2->GetType() == xmlCharacterData) {
								...
								if (strstr(string.c_str(), "light-heavy") != NULL) {
									finalbar = 1;
								} else if (strstr(string.c_str(), "light-thin") != NULL) {
									thinthin = 1;
								}
								break;
							}
					...
					if (element->GetName() == "repeat") {
						...
						while (obj2 != NULL) {
							if (obj2->GetType() == xmlCharacterData) {
								...
								if (strstr(string.c_str(), "light-heavy") != NULL) {
									finalbar = 1;
								} else if (strstr(string.c_str(), "light-light") != NULL) {
									thinthin = 1;
								}
								break;
							}
	...
	// out << "{" << partdata[staffno][index].ticktime << "}";
		...
		if ((voice > 1) && ((lastnoteindex < 0) ||
			...
			if (voice == 2) {
				...
				if (partdata[staffno][index].ticktime >= partvoiceticktime1) {
