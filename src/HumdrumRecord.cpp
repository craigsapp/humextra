...
char* HumdrumRecord::getBibKey(Array<char>& buffer) {
	if (!isBibliographic()) {
		buffer.setSize(1);
		buffer[0] = '\0';
...
char* HumdrumRecord::getBibValue(Array<char>& buffer) {
	if (!isBibliographic()) {
		buffer.setSize(1);
		buffer[0] = '\0';
		return buffer.getBase();
...
int HumdrumRecord::isKern(int index) {
	return isExInterp(index, "**kern");
}
