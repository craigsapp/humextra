...
// Last Modified: Thu 12 Dec 2019 03:31:06 PM PST implement with STL
...
class LayoutParameters {
	public:
		...
		int          getSize       (void) {return (int)m_code.getSize(); }
		int          getCodeSize   (int index) {return (int)m_key[index].getSize();}
		string& getCode       (int index)      { return m_code[index];   }
		vector<string>& getKeys (int index)    { return m_key[index];    }
		vector<string>& getValues (int index)  { return m_value[index];  }
		string& getKey        (int i1, int i2) { return m_key[i1][i2];   }
		string& getValue      (int i1, int i2) { return m_value[i1][i2]; }
		int          appendCode    (const string& codeName);
		int          addKeyValue   (int index, const string& keystr,
									       const string& valuestr);
		int          hasKeyName    (int codeindex, const string& kname);
		int          getKeyValueInt(int codeindex, const string& kname);
...
int LayoutParameters::getKeyValueInt(int codeindex, const string& kname) {
	for (int i=0; i<(int)key[codeindex].size(); i++) {
		if (kname == m_key[codeindex][i].getBase()) {
			return stoi(value[codeindex][i]);
		}
	}
	return 0;
}
...
int LayoutParameters::hasKeyName(int codeindex, const string& kname) {
	for (int i=0; i<(int)key[codeindex].size(); i++) {
		if (kname == key[codeindex][i]) {
			return i+1;
		}
	}
	return 0;
...
int LayoutParameters::appendCode(const string& codeName) {
	m_code.resize(m_code.size() + 1);
	m_key.resize(m_key.size() + 1);
	m_value.resize(m_value.size() + 1);
	m_code.last().resize(codeName.size()+1);
	m_code.last() = codeName;
	m_key.last().clear();
	m_value.last().clear();
	return m_code.getSize() - 1;
}

int LayoutParameters::addKeyValue(int index, const string& keystr,
		const string& valuestr) {
...
void LayoutParameters::addEntry(LayoutParameters& param, int index) {
	m_code.resize(m_code.size() + 1);
	m_code.last() = param.getCode(index);
	...
	m_key.last().setSize(keycount);
	m_value.last().setSize(keycount);

	for (int i=0; i<keycount; i++) {
		m_key.last()[i] = param.getKey(index, i);
		m_value.last()[i] = param.getValue(index, i);
	}
}
...
int LayoutParameters::getParameter(int codeindex, const string& searchkey) {
	...
	for (int j=0; j<key[codeindex].getSize(); j++) {
		if (m_key[codeindex][j] == searchkey) {
			output = j;
			break;
		}
	}
...
void LayoutParameters::prepareCode(const string& xcode, const string& params) {
	...
	m_key.resize(m_key.size() + 1);
	m_value.resize(m_value.size() + 1);
	m_key.last().resize(tokens.size());
	m_value.last().resize(tokens.size());

	PerlRegularExpression pre2;

	for (int i=0; i<(int)tokens.size(); i++) {
		if (pre2.search(tokens[i], "^([^=]+)=?(.*)", "")) {
			m_key.last()[i] = pre2.getSubmatch();
			m_value.last()[i] = pre2.getSubmatch();
		} else {
			m_key.last()[i] = "";
			m_value.last()[i] = "";
		}
...
void  checkColor               (string& colorchar, MuseRecord& arecord,
									     const string& token, string& colorout);
void  setColorCharacters       (HumdrumFile& infile, string& colorchar,
									     string& colorout);
void  setNoteheadShape         (MuseRecord& arecord, const string& token);
...
char  getColorCategory         (const string& color);
...
RationalNumber getDurationNoDots(const string& input, const string& def);
void  processLyricsSpines      (vector<vector<int>>& spineinfo,
									     HumdrumFile& infile, const string& data);
void  verifyMuseDataFile       (const string& filename);
...
void  updateFileTimeStamps     (const string& filename,  Options& options);
...
void  setGracenoteVisualRhythm (MuseRecord& arecord, const string& token);
void  convertKernLONtoMusePS   (char* buffer, vector<string>& keys,
									     vector<string>& values, const string& token);
...
void  substituteReferenceRecord(string& string, const string& refstring,
...
int   printUnterminatedTies    (MuseData& tempdata, HumdrumFile& infile,
									     int line, int track);
void  printPrehangTie          (MuseRecord& arecord, const string& buffer,
									     HumdrumFile& infile, int row, int col,
									     int voice);
...
string Copyright       = "";  // used with --copyright option
...
int main(int argc, char** argv) {
	...
	DashState.clear();
	DashState.reserve(infile.getMaxTracks() + 1);
...
void analyzeTacet(vector<vector<int>>& tacet, HumdrumFile& infile) {
	...
	for (int i=0; i<(int)tacet.resize(); i++) {
		tacet[i].clear();
		tacet[i].reserve(maxtrack+1);
	}

	int startline = 0;
	int endline   = 0;
	vector<int> tacetState(maxtrack+1, 0);
	int tacetcount = 0;
	...
	if ((startline != 0) || (tacetcount > 1)) {
		for (int ii=startline; ii<endline; ii++) {
			for (int jj=0; jj<(int)tacetState.size(); jj++) {
				tacet[ii][jj] = !tacetState[jj];
			}
		}
	}
...
void getRscaleState(HumdrumFile& infile,
	...
	vector<RationalNumber> current(tracks+1, 1);
...
void analyzeForwardTieConditions(vector<vector<string>>& tiestate,
	...
	vector<string> currentState (infile.getMaxTracks() + 1);
	...
	if (debugQ) {
		for (int i=0; i<infile.getNumLines(); i++) {
			...
			cout << "FTIESTATE " << i + 1 << ": ";
			for (int j=0; j<tiestate[i].getSize(); j++) {
				for (int k=0; k<tiestate[i][j].getSize(); k++) {
					if (tiestate[i][j][k] != 0) {
						cout << j << ":"
...
void printFooter(MuseData& musedata, HumdrumFile& infile, const string& footertext) {
	...
	string separator = footertext[0];
	pre.sar(separator, "\\$", "\\$", "");   // escape $ character
	pre.sar(separator, "\\^", "\\^", "");   // escape ^ character
	pre.sar(separator, "\\*", "\\*", "");   // escape * character

	vector<string> tokens;
	pre.getTokensWithEmpties(tokens, separator, footertext+1);
...
void cleanFooterField(string& footerline, HumdrumFile& infile) {
	...
	while (pre.search(footerline, "(@\\{[^}]+\\})(\\{[^}]*\\})?", "")) {
		buf1.getBase() = pre.getSubmatch(1);
...
void substituteReferenceRecord(string& string, const string& refstring,
	...
	if (pre.search(value, "^(20\\d\\d)/0?(\\d+?)\\/0?(\\d+?)\\/?$", "")) {
		string buffer = pre.getSubmatch(3);
		int month = atoi(pre.getSubmatch(2));
		switch (month) 
			case  1:  buffer = " Jan"; break;
			case  2:  buffer = " Feb"; break;
			case  3:  buffer = " Mar"; break;
			case  4:  buffer = " Apr"; break;
			case  5:  buffer = " May"; break;
			case  6:  buffer = " Jun"; break;
			case  7:  buffer = " Jul"; break;
			case  8:  buffer = " Aug"; break;
			case  9:  buffer = " Sep"; break;
			case 10:  buffer = " Oct"; break;
			case 11:  buffer = " Nov"; break;
			case 12:  buffer = " Dec"; break;
		}
		buffer += " ";
...
void getReferenceValue(string& value, string& refkey,
	...
	for (int i=0; i<infile.getNumLines(); i++) {
		...
		if (infile[i].getBibKey(buffer) != refkey) {
			continue;
		}
		infile[i].getBibValue(buffer);
		value = buffer;
		if (refkey.substr(0, 3) == "COM") {
			flipNameComma(value);
		} else if (refkey.aubstr(0, 3) = "COA") {
			flipNameComma(value);
		}
		return;
	}
	...
	for (int i=0; i<infile.getNumLines(); i++) {
		...
		if (refkey.substr(0, 3) == "COM") {
			flipNameComma(value);
		} else if (refkey.substr(0, 3) == "COA", 3) {
			flipNameComma(value);
		}
		return;
	}
...
void printMuse2PsOptions(HumdrumFile& infile) {
	...
	globaldefaults << ends;
	string globals(strlen(globaldefaults.str().c_str())+1);
...
void addMovementDesignation(char* buffer, HumdrumFile& infile, int line) {
	...
	if (omdline >= 0) {
		strcat(buffer, "D:");
		string movementdesignation(strlen(pre.getSubmatch(1))+1);
...
void addWorkTitle(HumdrumFile& infile, MuseData& tempdata) {
	...
	if (otlline < 0) {
		...
		string strang(strlen(pre.getSubmatch(1))+1);
...
int getScoreStaffVerticalPos(int note, int line, int row,
	...
	string clef = "*clefG2";  // default clef if none specified.
...
int verifyPart(MuseData& part) {
	...
	string checksum(strlen(pre.getSubmatch(2))+1);
	...
	string oldmd5sum(strlen(pre2.getSubmatch(1))+1);
	...
	tstream << ends;
	string partdata(strlen(tstream.str().c_str())+1);
